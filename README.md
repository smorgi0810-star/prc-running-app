...
이 버전에는 Supabase Storage 연결 준비 파일이 포함되어 있습니다.
...
docs/SUPABASE_NEXT_STEPS.md
```

`.env.local`은 로컬 실행용이며 `.gitignore`에 의해 GitHub에는 올라가지 않습니다.


## Gallery Supabase Storage 적용

Memory Gallery 업로드는 `gallery` bucket으로 업로드되도록 연결되었습니다.
자세한 내용은 `docs/GALLERY_SUPABASE_APPLIED.md`를 확인하세요.


## 전체 Supabase Storage 연결

Gallery, Feed, Profile, Hero, Anniversary Timeline 업로드를 Supabase Storage와 연결했습니다.
세부 내용은 `docs/ALL_STORAGE_SUPABASE_APPLIED.md`를 확인하세요.
