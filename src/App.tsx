...
import { loadSharedState, saveSharedState } from "./lib/database";
import { uploadGalleryImage, uploadFeedMedia as uploadFeedMediaToStorage, uploadProfileImage, uploadHeroImage, uploadTimelineImage } from "./lib/storage";
...
function sharedFromState(state: AppState) {
  return {
    users: state.users,
    ...
    timelineSlides: state.timelineSlides,
    notices: state.notices,
  };
}

function mergeServerState(prev: AppState, shared: any): AppState {
  const merged: AppState = {
    ...prev,
    users: shared.users?.length ? shared.users : prev.users,
    runRecords: shared.runRecords?.length ? shared.runRecords : prev.runRecords,
    feedPosts: shared.feedPosts?.length ? shared.feedPosts : prev.feedPosts,
    runEvents: shared.runEvents?.length ? shared.runEvents : prev.runEvents,
    galleryPhotos: shared.galleryPhotos?.length ? shared.galleryPhotos : prev.galleryPhotos,
    heroSlides: shared.heroSlides?.length ? shared.heroSlides : prev.heroSlides,
    timelineSlides: shared.timelineSlides?.length ? shared.timelineSlides : prev.timelineSlides,
    notices: shared.notices?.length ? shared.notices : prev.notices,
  };
  return merged;
}
...
function getInitialYearMonth(): { year: string; month: string } {
...
function getFriendlyUploadError(error: unknown, fallback: string): string {
  ...
  if (message.includes("MB 이하")) return message;
  if (/bucket|not found/i.test(message)) return "Supabase Storage bucket을 찾을 수 없습니다. feed/gallery/profile/hero bucket을 확인해주세요.";
  if (/policy|permission|row-level security|unauthorized|forbidden|403/i.test(message)) return "Supabase Storage Policy 권한 문제입니다. storage.objects INSERT/SELECT 정책을 확인해주세요.";
...
export default function PRCRunningCrewFinalApp() {
  ...
  const [calendarYear, setCalendarYear] = useState(initialYearMonth.year);
  const [calendarMonthOnly, setCalendarMonthOnly] = useState(initialYearMonth.month);
  const [dashboardYear, setDashboardYear] = useState(initialYearMonth.year);
  const [dashboardMonthOnly, setDashboardMonthOnly] = useState(initialYearMonth.month);
  ...
  const [timelineUploading, setTimelineUploading] = useState<Record<string, boolean>>({});
  ...
  const [eventForm, setEventForm] = useState({ title: "", date: today(), ampm: "AM", hour: "07", minute: "00", location: "", mapUrl: "", description: "" });
  ...
  useEffect(() => {
    ...
    async function loadServerData(): Promise<void> {
      try {
        ...
        setState((prev) => {
          const merged = mergeServerState(prev, shared);
          save(merged);

          // 기존 사용자가 PC localStorage에 가지고 있던 회원/프로필/Hero/Timeline 데이터가
          // 서버에 아직 없는 최초 전환 상황이면, 병합된 전체 상태를 서버에 1회 업로드한다.
          void saveSharedState(sharedFromState(merged)).catch((error) =>
            console.warn("Supabase DB 초기 동기화 실패", error)
          );

          return merged;
        });
      } catch (error) {
        console.warn("Supabase DB 데이터를 불러오지 못했습니다.", error);
      } finally {
        if (mounted) setServerLoaded(true);
      }
    }
    void loadServerData();
    return () => {
  ...
  useEffect(() => {
    function refreshSharedData(): void {
      ...
      }).catch((error) => console.warn("Supabase DB 새로고침 실패", error));
    }
  ...
  function patch(next: AppState | ((prev: AppState) => AppState)): void {
    setState((prev) => {
      const updated = typeof next === "function" ? (next as (p: AppState) => AppState)(prev) : next;
      save(updated);
      if (serverLoaded) {
        void saveSharedState(sharedFromState(updated)).catch((error) => {
          console.error("Supabase DB 저장 실패", error);
          toast("서버 DB 저장에 실패했습니다. Supabase table/policy를 확인해주세요.");
        });
      }
      return updated;
    });
  ...
  async function handleFeedMediaUpload(file?: File): Promise<void> {
    if (!currentUser) {
      setTab("login");
      return;
    }
    ...
    const maxBytes = (isVideo ? 80 : 10) * 1024 * 1024;
    if (file.size > maxBytes) {
      toast(isVideo ? "동영상은 80MB 이하만 업로드할 수 있습니다." : "이미지는 10MB 이하만 업로드할 수 있습니다.");
      return;
    }

    try {
      setFeedUploading(true);
      toast(isVideo ? "Feed 동영상을 Supabase Storage에 업로드 중입니다..." : "Feed 사진을 Supabase Storage에 업로드 중입니다...");

      const mediaUrl = await uploadFeedMediaToStorage(file);

      setFeedMediaType(isVideo ? "video" : "image");
      setFeedImage(mediaUrl);

      toast("Feed 미디어 업로드가 완료되었습니다.");
    } catch (error) {
      console.error(error);
      toast(getFriendlyUploadError(error, "Feed 업로드 중 오류가 발생했습니다. Supabase 환경변수, bucket, policy를 확인해주세요."));
    } finally {
  ...
  async function handleProfileImageUpload(file?: File): Promise<void> {
    ...
    if (!file.type.startsWith("image/")) {
      toast("프로필은 이미지 파일만 업로드할 수 있습니다.");
      return;
    }

    try {
      setProfileUploading(true);
      toast("프로필 사진을 Supabase Storage에 업로드 중입니다...");
      ...
      toast("프로필 사진 업로드가 완료되었습니다.");
    } catch (error) {
      console.error(error);
      toast("프로필 사진 업로드 중 오류가 발생했습니다.");
    } finally {
      setProfileUploading(false);
    }
  }

  async function handleHeroImageUpload(file?: File): Promise<void> {
    if (!currentUser?.isAdmin) return;
    if (!file) return;

    if (!file.type.startsWith("image/")) {
      toast("Hero 이미지는 이미지 파일만 업로드할 수 있습니다.");
      return;
    }

    try {
      setHeroUploading(true);
      toast("Hero 이미지를 Supabase Storage에 업로드 중입니다...");

      const imageUrl = await uploadHeroImage(file);
      setHeroImage(imageUrl);

      toast("Hero 이미지 업로드가 완료되었습니다.");
    } catch (error) {
      console.error(error);
      toast("Hero 이미지 업로드 중 오류가 발생했습니다.");
    } finally {
      setHeroUploading(false);
    }
  }

  async function handleTimelineImageUpload(slideId: string, file?: File): Promise<void> {
    if (!currentUser?.isAdmin) return;
    if (!file) return;

    if (!file.type.startsWith("image/")) {
      toast("Timeline 이미지는 이미지 파일만 업로드할 수 있습니다.");
      return;
    }

    try {
      setTimelineUploading((prev) => ({ ...prev, [slideId]: true }));
      toast("Anniversary Timeline 이미지를 Supabase Storage에 업로드 중입니다...");

      const imageUrl = await uploadTimelineImage(file);

      patch((p) => ({
        ...p,
        timelineSlides: p.timelineSlides.map((slide) =>
          slide.id === slideId ? { ...slide, image: imageUrl } : slide
        ),
      }));

      toast("Anniversary Timeline 이미지가 변경되었습니다.");
    } catch (error) {
      console.error(error);
      toast("Timeline 이미지 업로드 중 오류가 발생했습니다.");
    } finally {
      setTimelineUploading((prev) => ({ ...prev, [slideId]: false }));
    }
  }
  ...
  async function handleGalleryImageUpload(file?: File): Promise<void> {
    if (!currentUser) {
      setTab("login");
      return;
    }
    ...
    if (file.size > 10 * 1024 * 1024) {
      toast("이미지는 10MB 이하만 업로드할 수 있습니다.");
      return;
    }

    try {
      setGalleryUploading(true);
      toast("Memory Gallery 사진을 Supabase Storage에 업로드 중입니다...");
      ...
      toast("Memory Gallery 사진 업로드가 완료되었습니다.");
    } catch (error) {
      console.error(error);
      toast(getFriendlyUploadError(error, "Supabase Storage 업로드 중 오류가 발생했습니다. Bucket과 Policy를 확인해주세요."));
    } finally {
  ...
  function reset(): void {
    ...
    void saveSharedState(sharedFromState(productionEmptyState)).catch((error) =>
      console.warn("Supabase DB 초기화 저장 실패", error)
    );
  ...
  return <div className="min-h-screen bg-[#08080c] text-white">
    ...
    <main className="mx-auto max-w-7xl px-4 pb-28 pt-8 md:px-6">
      ...
      {activeTab === "feed" && <section><SectionTitle eyebrow="Instagram Mood" title="Running Feed" desc="러닝 날짜는 클릭 시 브라우저 달력으로 선택됩니다." />
        ...
        {showFeedForm && <Card className="mb-8 p-5"><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4"><Input label="거리(km)" type="number" value={recordForm.distanceKm} onChange={(v) => setRecordForm({ ...recordForm, distanceKm: v })} /><Input label="페이스" value={recordForm.pace} onChange={(v) => setRecordForm({ ...recordForm, pace: v })} placeholder="6:30" /><Input label="러닝 날짜" type="date" value={recordForm.date} onChange={(v) => setRecordForm({ ...recordForm, date: v })} /><Input label="러닝 시간(분)" type="number" value={recordForm.durationMinutes} onChange={(v) => setRecordForm({ ...recordForm, durationMinutes: v })} /></div><div className="mt-4 grid gap-4 md:grid-cols-2"><label><span className="mb-2 block text-xs font-black text-zinc-400">시작 시간: {minutesLabel(Number(recordForm.startMinutes))}</span><input type="range" min={0} max={1410} step={30} value={recordForm.startMinutes} onChange={(e) => setRecordForm({ ...recordForm, startMinutes: Number(e.target.value) })} className="w-full accent-lime-300" /></label><label><span className="mb-2 block text-xs font-black text-zinc-400">평균 심박수: {recordForm.avgHeartRate} bpm</span><input type="range" min={80} max={210} value={recordForm.avgHeartRate} onChange={(e) => setRecordForm({ ...recordForm, avgHeartRate: Number(e.target.value) })} className="w-full accent-rose-400" /></label></div><div className="mt-4 grid gap-4 md:grid-cols-2"><Textarea label="설명" value={recordForm.memo} onChange={(v) => setRecordForm({ ...recordForm, memo: v })} /><div className="grid gap-3"><Input label="해시태그" value={recordForm.hashtags} onChange={(v) => setRecordForm({ ...recordForm, hashtags: v })} placeholder="마라톤, 완주, PRC" /><label className="flex cursor-pointer justify-center rounded-2xl bg-white px-4 py-3 text-sm font-black text-zinc-950">📎 사진/동영상 첨부<input className="hidden" type="file" accept="image/*,video/mp4,video/quicktime" onChange={(e) => void handleFeedMediaUpload(e.target.files?.[0])} /></label></div></div>{feedImage && (feedMediaType === "video" ? <video src={feedImage} controls className="mt-4 max-h-80 w-full rounded-3xl object-cover" /> : <img src={feedImage} alt="preview" className="mt-4 max-h-80 w-full rounded-3xl object-cover" />)}<button onClick={addFeed} className="mt-5 w-full rounded-2xl bg-lime-300 px-5 py-4 text-sm font-black text-zinc-950">러닝 기록 등록</button></Card>}
      ...
      {activeTab === "gallery" && <section><SectionTitle eyebrow="Memory Gallery" title="전체 사진첩" desc="사진은 Supabase Storage gallery bucket에 저장되고, 앱에는 public URL이 기록됩니다." /><Card className="mb-8 p-5"><div className="grid gap-4 md:grid-cols-[1fr_1fr_auto]"><label className="flex cursor-pointer items-center justify-center rounded-2xl bg-white px-4 py-3 text-sm font-black text-zinc-950">사진 업로드<input type="file" accept="image/*,video/mp4,video/quicktime" className="hidden" onChange={(e) => void handleGalleryImageUpload(e.target.files?.[0])} /></label><Input label="사진 설명" value={galleryForm.caption} onChange={(v) => setGalleryForm({ ...galleryForm, caption: v })} /><button onClick={addGallery} disabled={galleryUploading} className="rounded-2xl bg-lime-300 px-6 py-3 text-sm font-black text-zinc-950 disabled:cursor-not-allowed disabled:opacity-50">{galleryUploading ? "업로드 중..." : "등록"}</button></div>{galleryForm.image && <img src={galleryForm.image} className="mt-4 max-h-96 w-full rounded-3xl object-cover" alt="gallery preview" />}</Card><div className="columns-2 gap-4 md:columns-3 lg:columns-4">{sortedGallery.map((p) => <div key={p.id} className="mb-4 break-inside-avoid overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04]"><button type="button" onClick={() => setGalleryPreview(p)} className="block w-full"><img src={p.image} className="w-full object-cover transition hover:scale-[1.02]" alt={p.caption} /></button><div className="p-4"><div className="font-black">{p.caption}</div><div className="mt-1 text-xs text-zinc-500">by {getUser(state.users, p.uploadedBy)?.nickname}</div></div></div>)}</div></section>}
