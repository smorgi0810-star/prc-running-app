...
export default function PRCRunningCrewFinalApp() {
  ...
  function addFeed(): void {
    ...
    const post: FeedPost = { id: uid("p"), userId: currentUser.id, date: recordForm.date, content: recordForm.memo || "오늘도 PRC와 함께 달렸습니다.", image: feedImage, hashtags: tags, likes: [], comments: [], runRecordId: rid };
  ...
  return <div className="min-h-screen bg-[#08080c] text-white">
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-3 md:px-6">
        <div className="flex items-center justify-between gap-3">
          ...
          {currentUser ? <div className="flex items-center gap-2">
            <button onClick={() => setTab("profile")} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[0.05] px-3 py-2 transition hover:bg-white/10"><Avatar user={currentUser} size="sm" /><span className="hidden text-sm font-black sm:inline">{currentUser.nickname}</span></button>
          ...
          </div> : <button onClick={() => setTab("login")} className="rounded-2xl bg-lime-300 px-4 py-2 text-sm font-black text-zinc-950">Login</button>}
        </div>
        <nav className="hidden items-center justify-center gap-1 rounded-3xl border border-white/10 bg-white/[0.04] p-1 xl:flex">
          {nav.map((n) => <button key={n.key} onClick={() => setTab(n.key)} className={cn("flex items-center gap-2 rounded-2xl px-4 py-2 text-sm font-black transition", tab === n.key ? "bg-white text-zinc-950" : "text-zinc-400 hover:bg-white/10 hover:text-white")}><span>{n.icon}</span>{n.label}</button>)}
        </nav>
      </div>
    </header>
    ...
    <main className="mx-auto max-w-7xl px-4 pb-28 pt-8 md:px-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <div className="relative min-h-[440px] overflow-hidden rounded-[32px] bg-zinc-900">
              ...
              <div className="relative z-10 flex min-h-[440px] flex-col justify-end p-7 md:p-10">
                ...
                <div className="mt-7 flex gap-3"><button onClick={() => setTab("feed")} className="rounded-2xl bg-lime-300 px-5 py-3 text-sm font-black text-zinc-950">피드 보기</button><button onClick={() => setTab("schedule")} className="rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-sm font-black">일정 보기</button></div>
              </div>
      ...
      {tab === "feed" && <section><SectionTitle eyebrow="Instagram Mood" title="Running Feed" desc="러닝 날짜는 클릭 시 브라우저 달력으로 선택됩니다." />
    ...
    <nav className="fixed bottom-4 left-1/2 z-50 flex max-w-[95vw] -translate-x-1/2 gap-1 overflow-x-auto rounded-full border border-white/10 bg-zinc-950/90 p-2 shadow-2xl xl:hidden">{nav.slice(0, 7).map((n) => <button key={n.key} onClick={() => setTab(n.key)} className={cn("rounded-full p-3 text-lg", tab === n.key ? "bg-lime-300 text-zinc-950" : "text-zinc-400")}>{n.icon}</button>)}</nav>
  </div>;
}
